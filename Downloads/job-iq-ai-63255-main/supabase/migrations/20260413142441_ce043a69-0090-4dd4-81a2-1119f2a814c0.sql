...
BEGIN
  IF NEW.status = 'completed' AND (OLD.status IS DISTINCT FROM 'completed') THEN
    ...
    v_base_amount := CASE NEW.domain
      WHEN 'medical' THEN 8.00
      WHEN 'legal' THEN 7.00
      WHEN 'finance' THEN 7.00
      WHEN 'code' THEN 6.00
      ELSE 6.00
    END;
    ...
    INSERT INTO public.annotation_payments (
      annotator_id, task_id, base_amount, status, time_spent_seconds, paid_at
    ) VALUES (
      NEW.assigned_annotator_id, NEW.id, v_base_amount, 'paid',
      EXTRACT(EPOCH FROM (NEW.completed_at - NEW.assigned_at))::INT, now()
    )
    ...
    IF v_existing_payment_id IS NULL THEN
      ...
      IF v_project_id IS NOT NULL THEN
        ...
        SET completed_tasks = COALESCE(completed_tasks, 0) + 1, updated_at = now()
    ...
    IF v_user_id IS NOT NULL AND v_expert_id IS NOT NULL AND v_existing_transaction_id IS NULL THEN
      INSERT INTO public.expert_transactions (
        expert_id, user_id, amount, type, status, description, task_id
      ) VALUES (
        v_expert_id, v_user_id, v_base_amount, 'task_credit', 'completed',
        'Tâche validée en ' || NEW.domain || ' — ' || v_base_amount || '$', NEW.id
      ) ON CONFLICT (task_id) DO NOTHING;

      INSERT INTO public.expert_balances (expert_id, user_id, available_balance, pending_balance, total_earned)
      VALUES (v_expert_id, v_user_id, v_base_amount, 0, v_base_amount)
      ON CONFLICT (user_id) DO UPDATE
      SET expert_id = COALESCE(public.expert_balances.expert_id, EXCLUDED.expert_id),
          available_balance = COALESCE(public.expert_balances.available_balance, 0) + EXCLUDED.available_balance,
